...
export default function Reviews({ listingId, reviews = [], onReviewAdded }) {
  ...
  return (
    <div className="mt-10 bg-white rounded-2xl shadow-xl p-8">
      <h2 className="text-2xl font-bold text-orange-700 mb-6">Reviews & Ratings</h2>
      
      {/* List Existing Reviews */}
      <div className="mb-8 space-y-4">
        {reviews.length > 0 ? (
          reviews.map((rev, index) => (
            <div key={rev._id || index} className="p-4 border border-orange-100 rounded-xl bg-orange-50/50">
              <div className="flex items-center gap-2 mb-2">
                <Rating value={rev.rating} readOnly size="small" sx={{ color: "#ea580c" }} />
